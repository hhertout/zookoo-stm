...
impl TlsMetrics {
    ...
    pub fn to_logfmt(&self) -> String {
        format!(
            ...
            self.cert_expiration_date.map_or("unknow".to_string(), |v| v.to_string()),
            self.cert_begin_date.map_or("unknow".to_string(), |v| v.to_string()),
            self.version
        )
    }
}
...
pub async fn inspect_tls(url: &str, cx: Context) -> Result<TlsMetrics, Box<dyn std::error::Error>> {
    ...
    let addr = lookup_host((host, port)).await?.find(|addr| addr.is_ipv4()).ok_or("no_ipv4")?;
    ...
    let cert = tls_stream.get_ref().peer_certificate()?.ok_or("no certificate")?;
...
fn x509_name_to_string(name: &X509NameRef) -> String {
    name.entries()
        .map(|e| {
            ...
            let value =
                e.data().as_utf8().map(|v| v.to_string()).unwrap_or_else(|_| "???".to_string());
            format!("{}={}", key, value)
        })
        .collect::<Vec<_>>()
        .join(", ")
